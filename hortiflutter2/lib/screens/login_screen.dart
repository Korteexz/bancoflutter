...
class _LoginScreenState extends State<LoginScreen>
  ...
  final loginScreen = TextEditingController();
  ...
  @override
  void initState(){ //iniciar a animação nas caixas de texto (input)
    super.initState();
 ...
 @override
 void dispose(){//encerrar a animação nos inputs
  loginScreen.dispose();
  senhaScreen.dispose();
  _controller.dispose();
  super.dispose();
 ...
 void fazerLogin(){
  ...
  if (loginScreen.text==usuarioCadastrado!.login &&
        senhaScreen.text==usuarioCadastrado!.senha) {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(builder: (_) => HomeScreen(nome:usuarioCadastrado!.nome),
            ),
          );
  }else{
    ScaffoldMessenger.of(context).showSnackBar(
